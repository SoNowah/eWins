...
class ListeTournois
{
    ...
    public function __construct() {
        ...
        $this->tournament2 = new MyTournament();
        ...
        if (isset($_POST['form__button__submit'])) {
            if ($_POST['form__listeTournois__statut'] == "all" && $_POST['form__listeTournois__sport'] == "all") {
                $this->participations = $this->participesRepository->getParticipationByUserId($user->id_utilisateur, $this->message);
                $this->allTournaments = $this->tournamentRepository->getAllTournament($this->message);
            } elseif($_POST['form__listeTournois__statut'] == "all") {
                $this->participations = $this->participesRepository->getParticipationByUserIdAndSportId($user->id_utilisateur, $_POST['form__listeTournois__sport'], $this->message);
                $this->allTournaments = $this->tournamentRepository->getAllTournamentBySportId($_POST['form__listeTournois__sport'], $this->message);
            } elseif ($_POST['form__listeTournois__sport'] == "all") {
                $this->participations = $this->participesRepository->getParticipationByUserIdAndStatutId($user->id_utilisateur, $_POST['form__listeTournois__statut'], $this->message);
                $this->allTournaments = $this->tournamentRepository->getAllTournamentByStatutId($_POST['form__listeTournois__statut'], $this->message);
            } else {
                $this->participations = $this->participesRepository->getParticipationByUserIdAndStatutIdAndBySportId($user->id_utilisateur, $_POST['form__listeTournois__statut'], $_POST['form__listeTournois__sport'], $this->message);
                $this->allTournaments = $this->tournamentRepository->getAllTournamentByStatutIdAndBySportId($_POST['form__listeTournois__statut'], $_POST['form__listeTournois__sport'], $this->message);
            }
        } else {
            $this->participations = $this->participesRepository->getParticipationByUserId($user->id_utilisateur, $this->message);
            $this->allTournaments = $this->tournamentRepository->getAllTournament($this->message);
        ...
        if (count($this->allTournaments) > 0) {
            ...
            foreach ($this->allTournaments as $allTournament) {
                $this->tournament2->id_tournoi = $allTournament->id_tournoi;
                $this->tournament2->nom = $this->tournamentRepository->getNom($this->tournament2->id_tournoi, $this->message);
                $this->tournament2->id_sport = $this->sportRepository->getSportIdByTournamentId($this->tournament2->id_tournoi, $this->message);
                $this->tournament2->placesDispo = $this->tournamentRepository->getPlacesDispo($this->tournament2->id_tournoi, $this->message);
                $this->tournament2->statut = $this->statutRepository->getStatutIdByTournamentId($this->tournament2->id_tournoi, $this->message);
                $this->tournament2->dateTournoi = $this->tournamentRepository->getDateTournoi($this->tournament2->id_tournoi, $this->message);
                $this->tournament2->dateFinInscription = $this->tournamentRepository->getDateFinInscription($this->tournament2->id_tournoi, $this->message);
                $this->tournament2->estActif = $this->tournamentRepository->getEstActif($this->tournament2->id_tournoi, $this->message);
                $this->tournament2->id_organisateur = $this->tournamentRepository->getOrganisateur($this->tournament2->id_tournoi, $this->message);
            }
        } else {
    ...
    public function get_nePasAfficher() {
        return $this->nePasAfficher;
    }
    ...
    public function get_tournament2() {
        return$this->tournament2;
    }
